...
const KarmaScoreChart = ({ score }) => {
  // Add state to track the animated score
  const [animatedScore, setAnimatedScore] = useState(0);
  
  // Determine color based on score
  const getKarmaScoreColorClass = (score) => {
    if (score >= 600) return 'karma-score-high';
    if (score >= 400) return 'karma-score-medium3';
  ...
  return (
    <div className="karma-chart-container">
      <svg className="karma-chart-circle" viewBox="0 0 64 64">
        ...
        />
        <circle
          className={`karma-chart-progress ${getKarmaScoreColorClass(score)}`}
          cx="32"
          cy="32"
          r={radius}
      ...
      <div className={`karma-chart-score ${getKarmaScoreColorClass(score)}`}>
        {animatedScore}
      </div>
    </div>
  );
